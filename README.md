...
5. **Energy Sensor (Optional)**: Select your smart plug's *cumulative* energy entity (Wh / kWh / MWh) if it exposes one. When set, each cycle's energy is taken from the meter's start→end delta instead of integrating the reported power samples - more accurate on plugs that only report power *on change* (which otherwise under-counts energy). Leave it empty to keep the built-in integrated estimate. It falls back to the integrated value automatically if the meter is unavailable, non-numeric, non-finite, does not advance, or is changed mid-cycle. This works best with a fine-resolution meter; a very coarse meter (e.g. whole-kWh steps) can still be less accurate than the integrated estimate on short cycles. Each stored cycle records which source was used via an `energy_source` field (`meter` or `integration`).
6. **Minimum Power (Optional)**: The standby threshold in Watts below which the appliance counts as off (default 2 W). Leave it unless your plug reports a high phantom load.
7. **Initial Profile (Optional)**: A second step lets you pre-create one profile - give it a name (e.g. "Cotton") and an approximate duration in minutes. Skip it and add profiles later from the panel.
