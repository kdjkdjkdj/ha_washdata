...
| **Playground** | Power-user tools: load a stored cycle onto an interactive power-trace canvas with **draggable detection thresholds**, scrub/play it back with live readouts, **test your settings across recent cycles** (detected/matched/ambiguous tallies), **sweep** any one setting to find its best-matching value, and inspect the profile envelope + DTW alignment to see exactly why a cycle matched. |
