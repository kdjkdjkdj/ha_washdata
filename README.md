...
- **Anti-Wrinkle Shield**: For dryers, washer-dryer combos, and washing machines, an optional shield keeps a completed cycle in an *Anti-Wrinkle* state during the periodic post-cycle anti-crease tumbles, so the finish notification fires when the program actually ends rather than after the drum stops or the door opens. Enable it per device under Advanced Settings. Unmatched anti-wrinkle cycles (no profile match) now end via a graceful timeout into `anti_wrinkle` once quiet for the configured `unmatched_off_delay`, instead of being force-stopped by the watchdog; tune the crease-blip threshold and unmatched-cycle timeout under Advanced Settings. **Known limitation**: for washing machines without a matched profile, this graceful end relies solely on `unmatched_off_delay` — there is no separate "spin seen" guard. A very long, low-power soak/prewash phase (all sub-`crease_resume_threshold` agitation lasting longer than `unmatched_off_delay`, default 2400 s / 40 min) can be mistaken for the post-program quiet period and trigger an early completion. If your washer's pre-wash/soak phases regularly run longer than that, raise `unmatched_off_delay` under Advanced Settings to cover them.
