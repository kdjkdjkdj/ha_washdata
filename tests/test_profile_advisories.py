...
def test_duration_trend_up_info():
    store = _store(
        ...
        trends={"Eco": {"duration_trend": "up", "duration_slope_pct": 0.12}},
    )
    adv = store.compute_profile_advisories()
    assert len(adv) == 1
...
def test_poor_health_suppresses_trend_advice_for_same_profile():
    ...
    store = _store(
        ...
        trends={"Cotton": {"duration_trend": "up", "duration_slope_pct": 0.2}},
    )
    adv = store.compute_profile_advisories()
    assert len(adv) == 1
    assert adv[0]["code"] == "poor_health"
...
def test_warnings_ranked_before_info():
    store = _store(
        ...
        trends={"Drift": {"duration_trend": "up", "duration_slope_pct": 0.1}},
    )
    adv = store.compute_profile_advisories()
