...
The first release since 0.4.5.2, and a large one — everything below is new relative to 0.4.5.2. Highlights: a full-screen management panel (everything the Settings options flow does, plus live dashboards, interactive editors, and per-user access control), a major matching-accuracy overhaul with near-duplicate profile grouping, notifications rebuilt around native Home Assistant automations, an experimental fully-gated NumPy-only ML subsystem that runs *alongside* — never replaces — the proven detection and matching code, and a complete panel localisation covering all 533 UI strings across 60 languages with native-quality domain-aware translation (settings section titles, field labels, field tooltips, artifact messages, and more).
...
- **Tools** (#281): Manual recording, learning-feedback resolution with a candidate comparison graph (one profile at a time overlaid on the cycle — replacing the old multi-profile colour spaghetti that was hard to read when a device had many profiles), phase-catalog management, diagnostics, and config export / import (paste JSON or load a file).
...
- **Profile groups** (#288): Group near-duplicate profiles (e.g. the same program at different temperatures, or lower-spin variants that run longer) so the matcher treats them as one candidate and then picks the best-fitting member. Cohesion is measured with a duration-tolerant DTW comparison of peak-normalised envelopes, so only genuinely similar profiles are collapsed; loose groups are left as individual profiles. The panel suggests near-duplicate clusters and provides a group control panel showing the aggregate envelope with each member overlaid, plus its cohesion. Safeguards keep an over-generic group from overpowering a genuinely better single profile (an ambiguous group win falls back to the uncertain/feedback path, and the chosen member must fit reasonably on its own). Together with the ambiguity gate (Smart Termination is now blocked whenever two profiles score too close to call), this directly fixes the class of split-cycle bugs where a shorter look-alike profile would match and trigger Smart Termination at its expected duration — turning one continuous wash cycle into two separate shorter ones.
...
- **Notifications built on native automations** (#287): Notifications are now driven by normal Home Assistant automations rather than a bespoke action editor. This also resolves the `'str' object has no attribute 'async_render_to_info'` error that occurred with complex notification actions containing `choose`, `if`, or `repeat` blocks — notification actions are now executed via HA's standard `Script` engine so template conditions compile correctly. Notifications gains an **Automations** section listing the automations that already use this device (deep-linking to the automation editor) plus a **New Automation** split button — a blank automation, or one prefilled with a *cycle started* / *cycle finished* event trigger for this device. WashData fires `ha_washdata_cycle_started` / `ha_washdata_cycle_ended` events and exposes entities, so side effects are built the same way as any other automation (see the [Notifications & Automations guide](NOTIFICATIONS.md)). Notify-service and people targets are compact **entity-pill pickers** with a domain-filtered add box and one-click removal, and the Android notification-channel fields suggest sensible channel names. Any custom actions carried over from an older setup keep firing and can be **converted to an automation** (or removed) from that section.

- **Experimental ML subsystem + dedicated ML Training tab (opt-in)**: A new NumPy-only ML subsystem, gated behind feature flags in `const.py`, runs *alongside* the proven detection and matching code and never replaces it. It ships embedded baseline models (cycle-end detection, curve quality, live-match commit), and all ML **management** is consolidated in a dedicated **ML Training** tab: training settings, training status and manual "Train now", matcher-tuning (with revert), and a **Use ML Models at Runtime** toggle. Per-cycle ML **health / review** stays inline in the Cycles tab and ML-calibrated tuning suggestions appear beside the classic ones. Optional on-device retraining only promotes a model when it beats the shipped baseline on held-out data. When a flag is off, the UI and logic stay inert.
...
- **Richer cycle and profile review**: The cycle-relabel view can overlay any of the device's profiles for quick visual comparison, per-cycle ML **quality / golden** review labels feed back into on-device training, and multi-select on the Cycles page was fixed so the checkbox reliably selects (previously it required clicking the profile name).
...
- **"Ran longer than usual" indicator**: A running cycle that materially exceeds its matched profile's typical duration is flagged as a soft, visible signal — a `cycle_anomaly`/`overrun_ratio` attribute on the state sensor and a ⏱ badge in the Cycles list. It never sends a notification and never ends or force-stops the cycle (the existing safety timeouts are unchanged).
...
- **Live phase tracks real progress**: The current-phase readout now maps the phase ranges you draw in the per-profile phase editor onto the live (ML-blended) progress rather than raw elapsed time, so it stays correct when a cycle runs longer or shorter than usual. One phase definition drives both the editor and the live estimate.
...
- **Panel fully localised (533 keys, 60 languages, native-quality)**: All UI strings in the management panel are surfaced through the panel's `_t()` translation system, served from `panel-translations.json` (rebuilt from `translations/en.json`). Scope: section titles, sub-group headers, individual field labels, field tooltip/doc strings, artifact detail messages, feedback count strings, button labels, dynamic chart labels, and the new-profile input label — 533 panel translation keys in total. All 60 language packs were reviewed by domain-aware contextual translation (not automated machine translation) and corrected for appliance-monitoring terminology. Machine translation (`translate.py`) is no longer used for panel keys; new keys must be translated by Claude subagents with explicit domain context so terminology stays correct across all languages. The config-flow translation files were also audited and updated to match the current options flow (removed ~40 obsolete multi-step flow keys, added 13 new keys for the reconfigure step and simplified options flow).
...
- If your device was set to Coffee Machine, Electric Vehicle, Heat Pump, or Oven, it is migrated to **Other (Advanced)** automatically (your tuned settings and profiles are kept). Since Other ships intentionally generic defaults, review the detection thresholds under Settings for that device.
