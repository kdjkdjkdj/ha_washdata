...
This release adds a full management UI for WashData and a major upgrade to matching accuracy. Everything that previously lived only in the Settings options flow is now also reachable from a dedicated full-screen panel, alongside live dashboards, interactive editors, and per-user access control. The options flow is unchanged and keeps working. Under the hood, the profile-matching pipeline was re-tuned end to end, near-duplicate profiles can be grouped, and an experimental, fully-gated NumPy-only ML subsystem was added *alongside* (never replacing) the proven detection and matching code.
...
- **Notification actions and smarter targets**: The custom Home Assistant **action configurator** is back for notifications - run any service call or action sequence on cycle start / finish / live progress, edited as guided rows or raw JSON. Notify-service and people fields are now compact **entity-pill pickers** with a domain-filtered add box and one-click removal instead of comma-separated text, and the Android notification-channel fields suggest sensible channel names.

- **ML Lab (experimental, opt-in)**: A new NumPy-only ML subsystem, gated behind feature flags in `const.py`, that runs *alongside* the proven detection and matching code and never replaces it. It ships embedded baseline models (cycle-end detection, curve quality, live-match commit), a shadow-mode comparison and per-cycle review panel, ML-calibrated tuning suggestions shown next to the classic ones, and optional on-device retraining that only promotes a model when it beats the shipped baseline on held-out data. When a flag is off, its UI and logic stay inert.

- **On-device matcher tuning**: When on-device training runs, WashData also tunes how much the matcher weighs curve shape versus duration and energy agreement, and how much its DTW ensemble leans on the derivative (DDTW) component - using the same held-out promotion discipline as the models. Device-specific weights are adopted only when they beat the shipped defaults on held-out cycles, and only bounded scoring weights can change (never structural matching behaviour). A new **Matching Tuning** card under Settings → ML Training shows the defaults, the in-use weights, when they were tuned, and a **Revert to defaults** button.
