...
The first release since 0.4.5.2, and a large one — everything below is new relative to 0.4.5.2. Highlights: a full-screen management panel (everything the Settings options flow does, plus live dashboards, interactive editors, and per-user access control — the options flow still works), a major matching-accuracy overhaul with near-duplicate profile grouping, notifications rebuilt around native Home Assistant automations, and an experimental, fully-gated, NumPy-only ML subsystem that runs *alongside* — never replaces — the proven detection and matching code.
