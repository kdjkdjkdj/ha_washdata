...
**F3 - Playground tab.** A new top-level Playground tab backed by `playground.py`, a headless replay harness that runs stored cycles through a genuine detector + matcher pass with overridden settings (concurrency 1-50). The UI is a single **interactive power-trace canvas**: load a stored cycle, drag the start/stop **threshold lines**, and scrub or play the run back to watch a live sensor strip (state, power, progress, time-left, energy, match). Below it, **Test on history** replays the most recent N cycles through the headless run with the staged overrides and reports detected/matched/ambiguous tallies + a plain-language verdict (`_pgRunSim`, chunked calls to `run_playground_simulation`); **Parameter sweep** loops one setting over a value grid client-side (reusing `run_playground_simulation` per value), charts the match rate, and offers a one-click "apply best" (`_pgRunSweep`/`_pgApplySweepBest`). The canvas overlays the profile envelope (`get_profile_envelope`) and the DTW alignment + full Stage-2/DTW/Stage-4 score breakdown (`get_dtw_debug`); it shows only real per-cycle scores (no synthetic confidences). Two WS commands: `run_playground_simulation` (in `_READ_WRITE_COMMANDS`) and `get_dtw_debug`. **Both require a full Home Assistant restart to register.** (The earlier three-sub-tab layout - Cycle Simulator / A/B Comparison / DTW Inspector - was consolidated into this one canvas; the sweep replaces the standalone A/B compare.)
