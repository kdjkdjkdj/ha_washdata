...
(`_pgPollTask`) covers backends/mocks without live subscription. Reprocess and
on-device ML training are the next consumers (same pattern; the infra already
supports them and the pills render any kind).
