...
const _SETTINGS_SECTIONS = [
  ...
  { id: 'detection', label: 'Detection', intro: 'How a cycle is detected as starting, running and finishing.', groups: [
    ...
    { sub: 'Cycle End', fields: [
      ...
      { key: 'prefix_guard_prefix_score', label: 'Prefix Guard Uses Prefix Score', type: 'checkbox', def: true,
        doc: 'Smart Termination is blocked while a much longer programme in the pool still looks like a plausible match, in case the running cycle is only its opening stretch. On: that candidate is scored over its first N seconds - the question the guard actually asks. Off: it is scored over its whole time-normalised curve (upstream behaviour), which a short cycle can clear even when the opening minutes look nothing alike.' },
...
class HaWashdataPanel extends HTMLElement {
  ...
  _pgOverrideFields() {
    return [
      ...
      ['prefix_guard_prefix_score','Prefix Guard Uses Prefix Score','', 'Score a longer look-alike over its first N seconds instead of its whole curve when deciding whether to block Smart Termination', 'advanced', 'bool'],
