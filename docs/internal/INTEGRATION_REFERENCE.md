...
|---|---|---|---|
| `manager.py` | 6845 | Central orchestrator (one per config entry). Power intake, matching orchestration, cycle-end pipeline, notification **delivery**, entity attributes, ML wiring, background ops. | [04](reference/04-manager-orchestrator.md) |
...
15. `[DOC]` `manager.py` "~5200 lines" -> actually 6845.
...
64. `[FIXED 2026-08-06]` **The trim spinner did nothing after snapping was added (regression from item 63)** (panel `_TRIM_STEP`/`_stepTrimBySample` + the trim input/keydown handlers). Snapping the boundary onto a real sample was correct, but the number input kept a `step` of 0.1 s. Every spinner click and arrow key therefore moved the value 0.1 s, which on a 10 s cadence always lands inside the snap radius of the sample already selected; the `change` handler snapped it straight back and the control looked dead. Reported from the live panel within minutes of the 0.5.3.10 rollout. **Root cause is conceptual, not arithmetic:** a trim boundary has no granularity finer than one sample, because the window is an inclusive filter and every offset between two samples behaves like the lower one. Arrow keys and spinner clicks now move by one SAMPLE (`_stepTrimBySample`), keyboard via an explicit `keydown` with `preventDefault` so the browser does not also apply its own step, mouse spinner via the `input` handler recognising a change of exactly one `_TRIM_STEP`. Free typing is unaffected and is still snapped on commit. Tests: 6 further `devtools/panel_smoke.js` checks, one of which asserts a step never leaves the boundary where it was, so the bounce cannot come back silently.

63. `[FIXED 2026-08-06]` **The cycle inspector let the user aim at a curve it was not applying the trim to (0.5.3.10)** (`ws_api.py:_downsample`/`_INSPECTOR_MAX_POINTS`/`ws_get_cycle_power_data`/`ws_analyze_split`, panel `_fmtTrimOffset`/`_snapToSample`/`_snapTrimBounds`/`_trimReadout` + trim handlers). Three defects in one chain, all of which had to be true for the observed harm: (a) `_downsample` decimated by striding, so single-sample events could vanish - measured, two of four 0 W samples lost from a real 376-sample curve, and an earlier occurrence produced a phantom 160 s gap plus a 120 s wrong trim target; (b) the reply carried no `sample_count`/`decimated`, so thinning was indistinguishable from a measurement gap; (c) the trim inputs rounded to whole seconds and nothing snapped, so a visible sample at 3132.3 s was entered as "3132" and `trim_cycle`'s inclusive `offset <= end_s` filter cut it. **Fixes:** min/max-per-bucket decimation (a zero is always its bucket minimum; unparseable rows dropped, never coerced to 0 W); a 2000-point budget for the two single-cycle endpoints so real curves reach the inspector whole (largest observed cycle: 1009 samples, ~14 KB); `sample_count`/`decimated` in both replies; tenth-second formatting and entry plus snapping to a real sample on drag-end, on change, and defensively before the WS call; a numeric kept/total + boundary-sample readout. Tests: `tests/test_curve_decimation.py` (9 cases, including one that asserts the OLD striding fails the zero-survival case so a revert is loud) and 5 `devtools/panel_smoke.js` checks pinned to the real 2026-08-06 offsets.

62. `[ADDED 2026-08-06]` **`heartbeat_interval` - optional stand-in readings for a power sensor that stops publishing entirely (0.5.3.9)** (`const.py:CONF_/DEFAULT_HEARTBEAT_INTERVAL`, `__init__.py` option setdefault, `manager.py:_start_heartbeat`/`_stop_heartbeat`/`_async_heartbeat_tick` + setup/reload/unload wiring, panel `timing` section, `translations/panel/en.json`). Completes item 61's family: the 0.5.3.7 report subscription covers a sensor that keeps publishing an unchanged value, this covers one that publishes nothing at all. When set (seconds, 0 = off, default off) a `async_track_time_interval` timer re-feeds the value the sensor still holds whenever no reading has arrived for that long, so the accumulators keep moving and `p95_dt` is pinned to a chosen cadence instead of the state-expiry keepalive rate. **Two invariants are the whole point of doing this in code rather than as a template sensor** (the workaround it replaces silences `Watchdog: Low power silence` and hid a 34.7 h MQTT outage on real hardware, 2026-08-04): (a) `_last_real_reading_time` is NOT advanced, so `no_update_active_timeout` still measures real silence; (b) a source whose own `last_reported` exceeds that timeout is treated as offline and gets no heartbeat, so a stale value never enters the curve with a fresh timestamp. Synthetic readings bypass `diag_buffer` and the learning cadence stats by design; recording mode is skipped. The timer is entry-lifetime, not cycle-scoped, because a quiet sensor also delays *detection*. Tests: 9 cases in `tests/test_manager.py` (default-off, timer cadence, re-feed, the two invariants, recording/unavailable/fresh-reading skips).
